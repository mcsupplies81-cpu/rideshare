...
export default async function AdminDashboardPage() {
  ...
  const [{ count: totalRides }, { data: payments }, { count: driversOnline }, { count: newUsers }, { data: initialRides }, { count: pendingApprovals }, { data: todayPayments }, { data: regions }] = await Promise.all([
    ...
    supabase.from('regions').select('id,name').eq('is_active', true).limit(1),
  ])

  return <div className="space-y-6">
    <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6"><MetricCard label="Total Rides" value={totalRides ?? 0} /><MetricCard label="Total Revenue" value={`$${((payments ?? []) as any[]).reduce((s: number, p: any) => s + (p.amount ?? 0), 0).toFixed(2)}`} /><MetricCard label="Revenue Today" value={`$${((todayPayments ?? []) as any[]).reduce((s: number, p: any) => s + (p.amount ?? 0), 0).toFixed(2)}`} /><MetricCard label="Drivers Online" value={driversOnline ?? 0} /><MetricCard label="New Users (7d)" value={newUsers ?? 0} /><MetricCard label="Pending Approvals" value={pendingApprovals ?? 0} subtitle={pendingApprovals > 0 ? 'Needs review' : undefined} /></div>
    <QuickActions defaultRegionId={regions?.[0]?.id ?? ''} />
    {pendingApprovals > 0 && <Link href="/admin/drivers?approval_status=pending" className="text-purple-300">Review Drivers →</Link>}
