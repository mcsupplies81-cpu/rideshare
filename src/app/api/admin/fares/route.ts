...
export async function GET() {
  ...
  const [{ data: fare }, { data: pricing }] = await Promise.all([
  ...
  return NextResponse.json({ fare_settings: fare, vehicle_pricing: pricing ?? [] })
}
...
export async function POST(req: Request) {
  ...
  const s = await createServiceClient() as any
  await s.from('fare_settings').insert({
    base_fare: b.base_fare,
    per_mile_rate: b.per_mile_rate,
