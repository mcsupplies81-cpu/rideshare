...
export default function DriverDashboard() {
  ...
  useEffect(() => { void (async () => { const { data: u } = await supabase.auth.getUser(); if (!u.user) return; const [{ data: userRow }, { data: driverRow }] = await Promise.all([supabase.from('users').select('full_name').eq('id', u.user.id).single(), supabase.from('drivers').select('is_online,trial_ends_at').eq('id', u.user.id).single()]); setName(userRow?.full_name?.split(' ')[0] || 'Driver'); setIsOnline(!!driverRow?.is_online); if (driverRow?.trial_ends_at) { const d = Math.ceil((new Date(driverRow.trial_ends_at).getTime() - Date.now()) / 86400000); if (d > 0) setTrialDays(d) } const { data: rides } = await supabase.from('rides').select('final_fare,created_at,status').eq('driver_id', u.user.id).gte('created_at', new Date().toISOString().slice(0,10)).eq('status','completed'); const earnings = (rides || []).reduce((a,r)=>a+Number(r.final_fare||0),0); setSummary({rides: rides?.length || 0, earnings, hours: isOnline ? 1 : 0}) })() }, [supabase,isOnline])
